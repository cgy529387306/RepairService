...
public class AppHelper {
    ...
    public static int calDpi2px(int dpi) {
        ...
        if (sDisplayMetricsDensity < 0) {
            sDisplayMetricsDensity = BaseApplication.getInstance().getResources().getDisplayMetrics().density;
        }
    ...
    public static int getStatusHeight() {
        ...
        if (resourceId > 0) {
            statusHeight = BaseApplication.getInstance().getResources().getDimensionPixelSize(resourceId);
        }
        return statusHeight;
    ...
    public static int getNavigationBarHeight() {
        ...
        int resourceId = BaseApplication.getInstance().getResources().getIdentifier("navigation_bar_height", "dimen", "android");
        if (resourceId > 0) {
            navigationBarHeight = BaseApplication.getInstance().getResources().getDimensionPixelSize(resourceId);
        }
    ...
    public static ArrayList<String> getLauncherPackageNames() {
        ...
        List<ResolveInfo> infos = BaseApplication.getInstance().getPackageManager().queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
...
//	}
    ...
    public static int getCurrentVersion() {
        int versionCode = 0;
        try {
            PackageInfo info = BaseApplication.getInstance().getPackageManager()
    ...
    public static String getExternalStoragePath() {
        ...
        if (Helper.isEmpty(result)) {
            result = BaseApplication.getInstance().getCacheDir().getPath();
        }
        return result;
    }
    ...
    public static String getBaseCachePath() {
        ...
        try {
        	if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
        			&& Environment.getExternalStorageDirectory().canWrite()) {
        		result = BaseApplication.getInstance().getExternalCacheDir().getAbsolutePath().concat(File.separator);
        	}
        }catch (Exception e) {
        	e.printStackTrace();
        }
        if (Helper.isEmpty(result)) {
            result = BaseApplication.getInstance().getCacheDir().getPath().concat(File.separator);
        }
        return result;
    }
    ...
    public static String getBaseFilePath() {
        ...
        if (Helper.isEmpty(result)) {
            result = BaseApplication.getInstance().getFilesDir().getPath().concat(File.separator);
        }
        return result;
    }
    ...
    public static String getBaseFilePath(String type) {
        if (Helper.isEmpty(type)) {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && Environment.getExternalStorageDirectory().canWrite()) {
                result = BaseApplication.getInstance().getExternalFilesDir(type).getAbsolutePath().concat(File.separator);
            }
            if (Helper.isEmpty(result)) {
                result = BaseApplication.getInstance().getFilesDir().getPath().concat(File.separator);
            }
